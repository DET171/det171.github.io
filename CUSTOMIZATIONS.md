...
- **Summary header:** Flex layout with `font-mono`, full foreground text, two SVG icons using `mask-image` technique (hamburger icon left, chevron right). Chevron rotates 180° on `details[open]` via `cubic-bezier` transition.
...
- **Prefix:** `$ play` monospaced label using `--font-mono`.
